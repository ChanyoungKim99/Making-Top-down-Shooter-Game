...
public class Zombie : LivingEntity
{
    ...
    {
        // "좀비 생성기"가 생성한 좀비의 초기 능력치를 설정하기 위해 실행하는 메서드
        // 따라서 외부에서 사용할 수 있도록 public으로 지정
    ...
    private void OnTriggerStay(Collider other)      // 트리거 충돌이 일어나고 있는 동안 물리 갱신주기에 맞춰 지속적으로 실행
    {
        ...
        if (!dead && Time.time >= lastAttackTime + timeBetAttack)
        {
            ...
            if (attackTarget != null && attackTarget == targetEntity) 
            {
                ...
                // 레이 캐스트를 사용하는 총과 달리 구체적인 타격 위치와 방향이 존재하지 않기 때문
                Vector3 hitPoint = other.ClosestPoint(transform.position);

                // 공격 대상위치에서 자신의 위치로 향하는 방향
                // = 자신의 위치 - 공격 대상의 위치
